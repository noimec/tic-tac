...
import { Either, matchEither } from "../lib/either";
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    { className, variant, size, asChild = false, error, children, ...props },
    ref,
  ) => {
      ...
      >
        ...
        {error &&
          matchEither(error, {
            right: () => null,
            left: (e) => (
              <div className="absolute left-0 top-[calc(100%_+_4px)] text-destructive">
                {e}
              </div>
            ),
          })}
